<?PHP
// $Id: proc-piv-ship.php 206 2009-03-17 02:01:45Z atlas $
...
if (isset($_POST["btn_adjudicate"]))
{
	if ($mysession->testmprocmask(PMASK_PRPSHIP) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, "piv", "ship", $dbh, $host);
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					if ((strcasecmp($newstat, "approved") == 0) || (strcasecmp($newstat, "rejected") == 0))
					{
						...
						if (strcasecmp($newstat, "approved") == 0)
						{
							if ($useremail !== false)
							{
								$mailtemplate = $mymail->getemailtemplate($_email_notification_templates, ET_RECEIVED_PIV);
					...
					if (strcasecmp($newstat, "pending") == 0)
					{
						...
						if ($useremail !== false)
						{
							$mailtemplate = $mymail->getemailtemplate($_email_notification_templates, ET_SHIPPED_PIV);
					...
					// Add object logs
					$ologentry = "[PIV SHIP] Modified to ".$newstat." by ".$ologname." on ".$dt;
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_PIVDELIV;
						$plentry[19] = "Applicant PIV shipping modified. Changed to ".$newstat;
