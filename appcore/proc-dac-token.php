<?PHP
// $Id: proc-dac-token.php 206 2009-03-17 02:01:45Z atlas $
...
if (isset($_POST["btn_adjudicate"]))
{
	if ($mysession->testmprocmask(PMASK_PRPTOKEN) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procdn = "procid=token,ounit=".$wflow.",".$uedn;
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, $wflow, "token", $dbh, $host);
										...
										elseif ($numspec == 5)
										{
											...
											if ($rlecode == 0)
											{
												if ($_xemenable)
												{
													...
													$plentry = array();
													$plentry[7] = EAID_DAC_TOKEN;
													$plentry[19] = "Applicant DAC token modified. Changed ".$attrname.($attrname == "xblk" ? " tag ".$srctag : "")." from ".$oldval." to ".$formval;
									...
									if (!$deldate)
									{
										...
										if ($rlecode == 0)
										{
											if ($_xemenable)
											{
												...
												$plentry = array();
												$plentry[7] = EAID_DAC_TOKEN;
												$plentry[19] = "Applicant DAC token modified. Changed ".$attrname.($attrname == "xblk" ? " tag ".$srctag : "")." from ".$oldval." to ".$mydate;
												$plentry[20] = $mysession->getmcid();
												$plentry[21] = $mysession->getucid();
												$plentry[33] = $mydate;
												$plentry[34] = $ologname;
										...
										if ($numspec == 4)
										{
											...
											if ($rlecode == 0)
											{
												if ($_xemenable)
												{
													...
													$plentry[7] = EAID_DAC_TOKEN;
													$plentry[16] = $tokencid;
													$plentry[19] = "Applicant DAC token modified. Deleted ".$attrname.($attrname == "xblk" ? " tag ".$srctag : "");
													$plentry[20] = $mysession->getmcid();
													$plentry[21] = $mysession->getucid();
													$plentry[34] = $ologname;
													$plentry[36] = $eparts[1];
