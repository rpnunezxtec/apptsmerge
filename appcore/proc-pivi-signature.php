<?PHP
// $Id: proc-pivi-signature.php 202 2009-03-16 02:52:57Z atlas $
...
{
	if ($mysession->testmprocmask(PMASK_PRPSIGNATURE) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procdn = "procid=signature,ounit=pivi,".$uedn;
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, "pivi", "signature", $dbh, $host);
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					// Add object logs
					$ologentry = "[PIVI SIGNATURE] Modified to ".$newstat." by ".$ologname." on ".$dt;
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_SIGNATURE;
						$plentry[19] = "Applicant document capture modified. Changed to ".$newstat;
