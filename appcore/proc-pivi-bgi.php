<?PHP
// $Id: proc-pivi-bgi.php 78 2008-11-03 23:13:28Z atlas $
		...
		if (($mysession->testmprocmask(PMASK_PRPBGIEND) === true) || ($mysession->testmprocmask(PMASK_PRPBGISTART) === true))
		{
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					if (strcasecmp($newstat, "initiated") == 0)
					{
						// manual initiation fills out the start attributes and the due date
						$procentry["startby"] = $acdn;
						$procentry["startdate"] = $dt;
						$ddt = date("YmdHis", (time()+15724800)).TIMEZONE;
						$procentry["duedate"] = $ddt;
					}

					// these can only be performed by the roles with end processmask
					if ($mysession->testmprocmask(PMASK_PRPBGIEND) === true)
					{
						if ((strcasecmp($newstat, "approved") == 0) || (strcasecmp($newstat, "rejected") == 0))
						{
							// manual approval or rejection fills out the end attributes and removes the due date
						...
						{
							// manual pending removes the start and end attributes and the duedate
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_BGIADJ;
						$plentry[19] = "Applicant ".strtoupper($wflow).":".strtoupper($category).":".strtoupper($chktype)." adjudicated. Changed to ".$newstat;
						$plentry[20] = $mysession->getmcid();
						$plentry[21] = $mysession->getucid();
						$plentry[33] = $newstat;
						$plentry[34] = $ologname;
			...
			if ($wauth)
			{
				...
				if(!$baddate && !$deldate)
				{
					...
					if (isset($_POST["yy_expdate"]))
						$yy = $_POST["yy_expdate"];
					else
						$yy = "";
					if (isset($_POST["mm_expdate"]))
						$mm = $_POST["mm_expdate"];
					else
						$mm = "";
					if (isset($_POST["dd_expdate"]))
						$dd = $_POST["dd_expdate"];
					else
						$dd = "";
					...
					{
						$baddate = true;
						print "<script type=\"text/javascript\">alert('Valid-to date cannot be in the past.')</script>\n";
					}
				}
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newval, $oldval) !== 0)
				{
					...
					$myprocess->updateprocmodby($procdn, $mysession->getmcid(), $dbh, $host);
					// Add to BGI historical log object for user
