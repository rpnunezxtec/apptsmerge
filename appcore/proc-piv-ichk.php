<?PHP
// $Id: proc-piv-ichk.php 202 2009-03-16 02:52:57Z atlas $
...
{
	if ($mysession->testmprocmask(PMASK_PRPICHK) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, "piv", "ichk", $dbh, $host);
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					// Add object logs
					$ologentry = "[PIV ICHK] Modified to ".$newstat." by ".$ologname." on ".$dt;
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_ICHK;
						$plentry[19] = "Applicant impersonation check modified. Changed to ".$newstat;
