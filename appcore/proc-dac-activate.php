<?PHP
// $Id: proc-dac-activate.php 291 2008-05-04 06:23:54Z atlas $
...
if (isset($_POST["btn_adjudicate"]))
{
	if ($mysession->testmprocmask(PMASK_PRPACTIVATE) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, "dac", "activate", $dbh, $host);
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					// Add object logs
					$ologentry = "[DAC ACTIVATE] Modified to ".$newstat." by ".$ologname." on ".$dt;
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_DAC_ACT;
						$plentry[16] = $tokencid;
						$plentry[19] = "Applicant DAC activation modified. Changed to ".$newstat;
