<?PHP
// $Id: proc-piv-signature.php 202 2009-03-16 02:52:57Z atlas $
...
if (isset($_POST["btn_adjudicate"]))
{
	if ($mysession->testmprocmask(PMASK_PRPSIGNATURE) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procdn = "procid=signature,ounit=piv,".$uedn;
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, "piv", "signature", $dbh, $host);
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					// Add object logs
					$ologentry = "[piv SIGNATURE] Modified to ".$newstat." by ".$ologname." on ".$dt;
