<?PHP
// $Id: proc-piv-fpcap.php 202 2009-03-16 02:52:57Z atlas $
...
{
	if ($mysession->testmprocmask(PMASK_PRPFPCAP) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					// Add object logs
					$ologentry = "[PIV FPCAP] Modified to ".$newstat." by ".$ologname." on ".$dt;
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_FPCAP;
						$plentry[19] = "Applicant fingerprint capture modified. Changed to ".$newstat;
