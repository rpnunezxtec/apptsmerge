<?PHP
// $Id: proc-piv-print.php 202 2009-03-16 02:52:57Z atlas $
...
if (isset($_POST["btn_adjudicate"]))
{
	if ($mysession->testmprocmask(PMASK_PRPPRINT) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, "piv", "print", $dbh, $host);
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					if (strcasecmp($newstat, "approved") == 0)
					{
						...
						if ($useremail !== false)
						{
							$mailtemplate = $mymail->getemailtemplate($_email_notification_templates, ET_PRINTED_PIV);
					...
					if (strcasecmp($newstat, "pending") == 0)
					{
						...
						if ($useremail !== false)
						{
							$mailtemplate = $mymail->getemailtemplate($_email_notification_templates, ET_PRINTING_PIV);
					...
					// Add object logs
					$ologentry = "[PIV PRINT] Modified to ".$newstat." by ".$ologname." on ".$dt;
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_PIVGEN;
						$plentry[19] = "Applicant PIV print modified. Changed to ".$newstat;
...
elseif (isset($_POST["btn_reprint"]))
{
	if ($mysession->testmprocmask(PMASK_REPRINT) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			// Add object logs
			$ologentry = "[PIV PRINT] Re-print change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			// Add to common log object for user
			$myldap->ldap_checkandcreate($cldn, "xsystem", array(), $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			...
			// Add object logs
			$ologentry = "[PIV ENCODE] Re-print change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			// Add to common log object for user
			$myldap->ldap_checkandcreate($cldn, "xsystem", array(), $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			...
			// Add object logs
			$ologentry = "[PIV SHIP] Re-print change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			// Add to common log object for user
			$myldap->ldap_checkandcreate($cldn, "xsystem", array(), $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			...
			// Add object logs
			$ologentry = "[PIV ACTIVATE] Re-print change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			// Add to common log object for user
			$myldap->ldap_checkandcreate($cldn, "xsystem", array(), $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
