<?PHP
// $Id: proc-piv-sponsor.php 386 2009-05-27 02:46:37Z  $
...
include("../appconfig/config-piv-sponsor.php");
...
if (isset($_POST["btn_sponsor"]))
{
	if ($mysession->testmprocmask(PMASK_SPONSOR) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procdn = "procid=sponsor,ounit=piv,".$uedn;
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, "piv", "sponsor", $dbh, $host);
			...
			// Add object logs
			$ologentry = "[PIV SPONSOR] Sponsored by ".$ologname." on ".$dt;
			...
			$myprocess->checkcreateprocessobject($uedn, "piv", "fpcap", $dbh, $host);
			$myprocess->checkcreateprocessobject($uedn, "piv", "doccap", $dbh, $host);
			$myprocess->checkcreateprocessobject($uedn, "piv", "photocap", $dbh, $host);
			$myxld->xld_modify($dbh, "procid=fpcap,ounit=piv,".$uedn, $enrolentry);
			$myprocess->updateprocmodby("procid=fpcap,ounit=piv,".$uedn, $mysession->getmcid(), $dbh, $host);
			
			$myxld->xld_modify($dbh, "procid=doccap,ounit=piv,".$uedn, $enrolentry);
			$myprocess->updateprocmodby("procid=doccap,ounit=piv,".$uedn, $mysession->getmcid(), $dbh, $host);

			$myxld->xld_modify($dbh, "procid=photocap,ounit=piv,".$uedn, $enrolentry);
			$myprocess->updateprocmodby("procid=photocap,ounit=piv,".$uedn, $mysession->getmcid(), $dbh, $host);
			
			// email to the applicant
			if ($useremail !== false)
			{
				$mailtemplate = $mymail->getemailtemplate($_email_notification_templates, ET_SPONSOR_PIV);
				
				// Get info to be substituted into message body
				$mailtags = array();
				$mailtags["workflow"] = "PIV";
				$dnset = array();
		...
		if ($dbh !== false)
		{
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					// Add object logs
					$ologentry = "[PIV SPONSOR] Modified to ".$newstat." by ".$ologname." on ".$dt;
					$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
					// Add to common log object for user
					$myldap->ldap_checkandcreate($cldn, "xsystem", array(), $host, $dbh);
					$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
...
elseif (isset($_POST["btn_reissue"]))
{
	if ($mysession->testmprocmask(PMASK_REISSUE) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procdn = "procid=fpcap,ounit=piv,".$uedn;
			$procentry["status"] = "Pending";
			$myxld->xld_modify($dbh, $procdn, $procentry);
			$myprocess->updateprocmodby($procdn, $mysession->getmcid(), $dbh, $host);
			
			$ologentry = "[PIV FPCAP] Re-issue change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			
			$procdn = "procid=doccap,ounit=piv,".$uedn;
			$procentry["status"] = "Pending";
			$myxld->xld_modify($dbh, $procdn, $procentry);
			$myprocess->updateprocmodby($procdn, $mysession->getmcid(), $dbh, $host);
			
			$ologentry = "[PIV DOCCAP] Re-issue change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			
			$procdn = "procid=photocap,ounit=piv,".$uedn;
			$procentry["status"] = "Pending";
			$myxld->xld_modify($dbh, $procdn, $procentry);
			$myprocess->updateprocmodby($procdn, $mysession->getmcid(), $dbh, $host);
			
			$ologentry = "[PIV PHOTOCAP] Re-issue change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			
			$procdn = "procid=ichk,ounit=piv,".$uedn;
			$procentry["status"] = "Pending";
			$myxld->xld_modify($dbh, $procdn, $procentry);
			$myprocess->updateprocmodby($procdn, $mysession->getmcid(), $dbh, $host);
			
			$ologentry = "[PIV ICHK] Re-issue change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			
			$procdn = "procid=print,ounit=piv,".$uedn;
			$procentry["status"] = array();
			$myxld->xld_mod_del($dbh, $procdn, $procentry);
			$myprocess->updateprocmodby($procdn, $mysession->getmcid(), $dbh, $host);
			
			$ologentry = "[PIV PRINT] Re-issue change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			...
			$ologentry = "[PIV ENCODE] Re-issue change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			
			$procdn = "procid=ship,ounit=piv,".$uedn;
			$procentry["status"] = array();
			$myxld->xld_mod_del($dbh, $procdn, $procentry);
			$myprocess->updateprocmodby($procdn, $mysession->getmcid(), $dbh, $host);
			
			$ologentry = "[PIV SHIP] Re-issue change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			...
			$ologentry = "[PIV ACTIVATE] Re-issue change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			
			$procdn = "procid=hspd12,ounit=piv,".$uedn;
			$procentry["status"] = array();
			$procentry["pdn"] = array();
			$myxld->xld_mod_del($dbh, $procdn, $procentry);
			$myprocess->updateprocmodby($procdn, $mysession->getmcid(), $dbh, $host);
				
			$ologentry = "[PIV HSPD12] Re-issue change by ".$ologname." on ".$dt;
			$mylog->addobjlog($procdn, $ologentry, $host, $dbh);
			$mylog->addobjlog($cldn, $ologentry, $host, $dbh);
			...
			if ($useremail !== false)
			{
				$mailtemplate = $mymail->getemailtemplate($_email_notification_templates, ET_REISSUE_PIV);
