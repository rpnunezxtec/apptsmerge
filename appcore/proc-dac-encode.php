<?PHP
// $Id: proc-dac-encode.php 202 2009-03-16 02:52:57Z atlas $
...
if (isset($_POST["btn_adjudicate"]))
{
	if ($mysession->testmprocmask(PMASK_PRPENCODE) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, "dac", "encode", $dbh, $host);
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					// Add object logs
					$ologentry = "[DAC ENCODE] Modified to ".$newstat." by ".$ologname." on ".$dt;
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_DAC_ENCODE;
						$plentry[19] = "Applicant DAC encoding modified. Changed to ".$newstat;
