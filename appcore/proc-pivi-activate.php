<?PHP
// $Id: proc-pivi-activate.php 202 2009-03-16 02:52:57Z atlas $
...
if (isset($_POST["btn_adjudicate"]))
{
	if ($mysession->testmprocmask(PMASK_PRPACTIVATE) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, "pivi", "activate", $dbh, $host);
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					// Add object logs
					$ologentry = "[PIV-I ACTIVATE] Modified to ".$newstat." by ".$ologname." on ".$dt;
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_PIVI_ACT;
						$plentry[16] = $tokencid;
						$plentry[19] = "Applicant PIV-I activation modified. Changed to ".$newstat;
