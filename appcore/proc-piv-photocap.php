<?PHP
// $Id: proc-piv-photocap.php 202 2009-03-16 02:52:57Z atlas $
...
{
	if ($mysession->testmprocmask(PMASK_PRPPORTRAIT) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					// Add object logs
					$ologentry = "[PIV PHOTOCAP] Modified to ".$newstat." by ".$ologname." on ".$dt;
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_PHOTOCAP;
						$plentry[19] = "Applicant portrait capture modified. Changed to ".$newstat;
