<?PHP
// $Id: proc-pivi-encode.php 202 2009-03-16 02:52:57Z atlas $
...
if (isset($_POST["btn_adjudicate"]))
{
	if ($mysession->testmprocmask(PMASK_PRPENCODE) === true)
	{
		...
		if ($dbh !== false)
		{
			...
			$procentry = array();
			$myprocess->checkcreateprocessobject($uedn, "pivi", "encode", $dbh, $host);
			...
			if ($wauth)
			{
				...
				if (strcasecmp($newstat, $oldstat) !== 0)
				{
					...
					// Add object logs
					$ologentry = "[PIV-I ENCODE] Modified to ".$newstat." by ".$ologname." on ".$dt;
					...
					if ($_xemenable)
					{
						...
						$plentry = array();
						$plentry[7] = EAID_PIVI_ENCODE;
						$plentry[19] = "Applicant PIV-I encoding modified. Changed to ".$newstat;
