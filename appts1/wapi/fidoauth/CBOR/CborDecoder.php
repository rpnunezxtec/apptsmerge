...
include_once("binary/ByteBuffer.php");
...
class CborDecoder 
{
    ...
    public static function decode($bufOrBin) 
	{
        ...
        if ($offset !== $buf->getLength()) {
            throw new Error(403,'Unused bytes after data item.');
        }
        return $result;
    }
    ...
    protected static function _parseFloatSimple($val, ByteBuffer $buf, &$offset) {
        switch ($val) {
            ...
            case 29:
            case 30:
                throw new Error(403,'Reserved value used.');

            case 31:
                throw new Error(403,'Indefinite length is not supported.');
        }
    ...
    protected static function _parseSimple($val) {
        ...
        throw new Error(403,sprintf('Unsupported simple value %d.', $val));
    }
    ...
    protected static function _parseItemData($type, $val, ByteBuffer $buf, &$offset) {
        ...
        throw new Error(403, sprintf('Unknown major type %d.', $type));
    }
    ...
    protected static function _parseMap(ByteBuffer $buf, &$offset, $count) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (!\is_int($mapKey) && !\is_string($mapKey)) {
                throw new Error(403,'Can only use strings or integers as map keys');
            }
