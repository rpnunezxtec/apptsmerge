...
class fido
{
	...
	{
		$rv = array();
		$rv["publicKey"]["rp"]["name"] = "Authentx PIV-I";
		$rv["publicKey"]["rp"]["id"] = $this->rpId;
		
		$rv["publicKey"]["authenticatorSelection"]["userVerification"] = "required";
		
		$rv["publicKey"]["user"]["id"] = new ByteBuffer(hex2bin($user_uuid));
		$rv["publicKey"]["user"]["displayName"] = $display_name;
		$rv["publicKey"]["user"]["name"] = $userid;
		
		$rv["publicKey"]["pubKeyCredParams"][0]["type"] = "public-key";
		$rv["publicKey"]["pubKeyCredParams"][0]["alg"] = -7;				// ES256 (ECDSA w/ SHA-256)
		$rv["publicKey"]["pubKeyCredParams"][1]["type"] = "public-key";
		$rv["publicKey"]["pubKeyCredParams"][1]["alg"] = -257;			// RS256 (RSASSA-PKCS1-v1_5 using SHA-256)
		
		$rv["publicKey"]["attestation"] = "direct";
		
		$rv["publicKey"]["timeout"] = "20000";
		
		// add suffix and prefix to parse on client side
		$rv["publicKey"]["challenge"] = $challenge;
		
		$rv["publicKey"]["extensions"]["exts"] = "true";
	...
	getdefaultchallangelogin($challange, $credids_array, $user_uuid)
	{
		...
		$rv["publicKey"]["timeout"] = "60000";
		$rv["publicKey"]["challenge"] = $challange;
		$rv["publicKey"]["userVerification"] = "required";
		...
		foreach($credids_array as $credid)
		{
			...
			$rv["publicKey"]["allowCredentials"][] = $allowed_cred;
		}
