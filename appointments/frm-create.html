...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_schedule') {
    session_start();
    ...
    if ($s_site->num_rows == 0) {
        echo '<p style="padding: 20px; color: red;">Site not found.</p>';
        exit;
    }
    ...
    $current_user_name = $myappt->session_getuuname();
    ...
    if ($s_current_user) {
        ...
        $current_user_email = $r_current_user['email'] ?? '';
        $current_user_phone = $r_current_user['phone'] ?? '';
        $current_user_component = $r_current_user['component'] ?? '';
        $s_current_user->free();
    }
    ...
    // Output the results
    echo '<div style="padding-bottom: 5px" class="nameheading">Appointment Date: ' . date('D M jS Y', $datestamp) . '</div>';
    echo '<div style="padding-bottom: 5px" class="nameheading">Appointment for: ' . htmlentities($current_user_name) . '</div>';
    echo '<div style="padding-bottom: 5px" class="nameheading">Site: ' . htmlentities($sitename) . '</div>';
    
    echo '<table id="appt_schedule" class="striped">';
    echo '<thead><tr class="light-xtec-blue">';
    echo '<th class="tableheader">Time</th><th class="tableheader">Name</th><th class="tableheader">Component</th>';
    echo '<th class="tableheader">Appt Ref</th><th class="tableheader">Reason</th>';
    echo '<th class="tableheader">Email</th><th class="tableheader">Phone</th>';
    echo '<th class="tableheader">Action</th>'; // Added Action column
    echo '</tr></thead><tbody>';
    
    if ($s_appt->num_rows == 0) {
        echo '<tr height="20"><td colspan="8" class="matrixline"><p>No Appointments</p></td></tr>';
    } else {
        while ($r_appt = $s_appt->fetch_assoc()) {
            $adt = $r_appt["starttime"];
            $atime = substr($adt, 11, 2) . ":" . substr($adt, 14, 2);
            $apptid = $r_appt['apptid'];
            $apptref = $r_appt['apptref'];
            $uid = $r_appt['uid'];
            
            echo '<tr height="30">';
            echo '<td><p>' . htmlentities($atime, ENT_QUOTES, 'UTF-8') . '</p></td>';
            echo '<td><p>' . htmlentities($current_user_name, ENT_QUOTES, 'UTF-8') . '</p></td>';
            echo '<td><p>' . htmlspecialchars($current_user_component) . '</p></td>';
            echo '<td><p>' . htmlentities($apptref, ENT_QUOTES, 'UTF-8') . '</p></td>';
            echo '<td><p>' . ($r_appt['apptrsn'] === '' ? '&nbsp;' : htmlentities($r_appt['apptrsn'], ENT_QUOTES, 'UTF-8')) . '</p></td>';
            echo '<td><p>' . htmlspecialchars($current_user_email) . '</p></td>';
            echo '<td><p>' . htmlspecialchars($current_user_phone) . '</p></td>';
            
            // Cancel button
            echo '<td style="text-align: center;">';
            echo '<button onclick="cancelAppointment(\'' . htmlspecialchars($apptid, ENT_QUOTES) . '\', \'' . htmlspecialchars($apptref, ENT_QUOTES) . '\', \'' . htmlspecialchars($uid, ENT_QUOTES) . '\')" ';
            echo 'style="padding: 6px 12px; background: #cf222e; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 12px;">Cancel</button>';
            echo '</td>';
...
// Handle AJAX request for cancelling appointment
if (isset($_GET['ajax']) && $_GET['ajax'] === 'cancel_appointment') {
    session_start();
    ...
    $q_check = "SELECT * FROM appointment WHERE apptid='".$dbh->real_escape_string($apptid)."' AND uid='".$dbh->real_escape_string($uid)."'";
    $s_check = $dbh->query($q_check);
    
    if (!$s_check || $s_check->num_rows == 0) {
        echo json_encode(['success' => false, 'message' => 'Appointment not found']);
        if ($s_check) $s_check->free();
        $dbh->close();
        exit;
    }
    
    $r_check = $s_check->fetch_assoc();
    $apptref = $r_check['apptref'];
    $s_check->free();
    
    // Delete the appointment
    $q_delete = "DELETE FROM appointment WHERE apptid='".$dbh->real_escape_string($apptid)."' AND uid='".$dbh->real_escape_string($uid)."' LIMIT 1";
    $s_delete = $dbh->query($q_delete);
    
    if ($s_delete && $dbh->affected_rows > 0) {
        // Try to log the cancellation (but don't fail if logging fails)
        try {
            $logstring = "Appointment ".$apptid." (ref: ".$apptref.") cancelled by user";
            $myappt->createlogentry($dbh, $logstring, $uid, 5); // Using 5 as a generic log type
        } catch (Exception $e) {
            // Logging failed but appointment was cancelled, so continue
        }
...
// Reset wizard if not coming from within the wizard workflow
// This preserves wizard state for all wizard interactions (buttons, navigation, slot selection)
// but resets when user navigates here from outside
if (!isset($_POST['circle1']) && !isset($_POST['circle2']) && !isset($_POST['circle3']) && 
    !isset($_POST['circle4']) && !isset($_POST['circle5']) && !isset($_POST['next_step']) &&
    !isset($_POST['submit_site']) && !isset($_GET['wk']) && !isset($_GET['site']) && 
    !isset($_GET['st']) && !isset($_GET['uid']) && !isset($_GET['step'])) {
    // No wizard-related parameters found - this is a fresh page load from outside
    unset($_SESSION['wizard_step']);
    unset($_SESSION['wizard_data']);
    unset($_SESSION['max_step_reached']);
}
...
// At the top of your file, merge GET into POST
if (isset($_GET['wk'])) {
    $_POST['wk'] = $_GET['wk'];
}
if (isset($_GET['site'])) {
    $_POST['site'] = $_GET['site'];
}
...
// Handle circle navigation
 for ($i = 1; $i <= 5; $i++) {
    if (isset($_POST["circle{$i}"])) {
        ...
        if ($i <= $_SESSION['max_step_reached']) {
...
// Handle "Next" button - ONLY if no circle was clicked
...
// Handle "Next" button
if (isset($_POST['next_step']) && !$circle_clicked) {
...
if($siteid !== false && $priv_apptsched !== false)
{
...
<div id="content">
	...
	?>
		...
		<div class="cards-scope">
			<div style="padding-bottom: 5%;" class="cards-grid">
				<!-- Card 1 -->
				<div class="card">
	...
	?>
		...
		<div style="display:flex;flex-direction:column;align-items:center;gap:28px;padding:10px 0;">
			...
			<div class="inputtitlerow3 inputtitlespacer3" style="display:flex;justify-content:center;align-items:flex-end;gap:24px;flex-wrap:nowrap; padding-bottom:0%">
	...
	?>
		...
		<p style="text-align:center"><b>Select a Date and Time</b> for your Appointment. All available Appointment times are represented in Site Local time. If you wish to view appointments booked for a particular date, select the date column above. Example: Thu Nov 6th</p>
		...
		<div style="display:flex;flex-direction:column;align-items:center;gap:28px;padding:10px 0;">
			...
			{			
				...
				{	
					...
					print "</tr>\n";
					print "<tr class=\"apptweekrow\">\n";
					print "<td width=\"100%\" class=\"apptweek apptweek-nav\">\n";
					print "<a href=\"".htmlentities($formfile)."?wk=".urlencode($wkstamp_prev)."&site=".urlencode($siteid)."\" title=\"Previous week\" class=\"nav-arrow\">‹</a>";
					print " <span class=\"current-week\">".htmlentities(date("jS M Y", $wkstamp))."</span> ";
					print "<a href=\"".htmlentities($formfile)."?wk=".urlencode($wkstamp_next)."&site=".urlencode($siteid)."\" title=\"Next week\" class=\"nav-arrow\">›</a>\n";
					print "</td>\n";
					print "</tr>\n";
					print "</table>\n";
					...
					for ($d = 0; $d < 7; $d++) {
						...
						if ($priv_apptsched) {
							...
							print "<td class=\"apptdayname\" width=\"".$slotwidth."%\" "
								."onmouseover=\"javascript:this.style.cursor='pointer';\" "
								."onclick=\"showDailySchedule(".$wkdayslotstartstamp.", '".htmlspecialchars($siteid, ENT_QUOTES)."', '".htmlspecialchars($mac, ENT_QUOTES)."',);\">"
								.$wkdate."</td>\n";
						} else {
		...
		<div id="daily-schedule-section" style="display: none;">
			<div id="daily-schedule-content">
				<!-- Content is loaded from the function showDailySchedule which then checks the $_GET[ajax] found all the way at the top of this file. -->
			</div>
		</div>

		<script>
			function showDailySchedule(datestamp, siteid, mac) 
			{
				document.getElementById('daily-schedule-content').innerHTML = '<div style="padding: 20px; text-align: center;">Loading appointments...</div>';
				document.getElementById('daily-schedule-section').style.display = 'block';
				
				fetch('frm-create.html?ajax=get_schedule&siteid=' + encodeURIComponent(siteid) + 
					'&datestamp=' + datestamp + 
					'&avc=' + encodeURIComponent(mac))
					.then(response => response.text())
					.then(html => {
						document.getElementById('daily-schedule-content').innerHTML = 
							'<div style="background:#f9f9f9; border:1px solid #d0d7de; border-radius:8px; max-width:1100px; margin:0 auto; display:flex; flex-direction:column; gap:16px; padding:20px;">' +
							'<div style="display:flex; flex-direction:column; align-items:stretch; gap:20px;">' +
							html +
							'</div></div>';
						
						// Scroll AFTER content is loaded - use setTimeout to ensure DOM is updated
						setTimeout(function() {
							document.getElementById('daily-schedule-section').scrollIntoView({ 
								behavior: 'smooth',
								block: 'start'
							});
						}, 100);
					...
					.catch(error => {
						document.getElementById('daily-schedule-content').innerHTML = '<p style="padding: 20px; color: red; text-align: center;">Error loading schedule.</p>';
					});
			}

			function cancelAppointment(apptid, apptref, uid) {
    if (confirm('Are you sure you want to cancel appointment ' + apptref + '?')) {
        // Show loading
        document.getElementById('daily-schedule-content').innerHTML = '<div style="padding: 20px; text-align: center;">Cancelling appointment...</div>';
        
        // Send cancel request
        fetch('frm-create.html?ajax=cancel_appointment&apptid=' + encodeURIComponent(apptid) + 
            ...
            .then(text => {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        document.getElementById('daily-schedule-section').style.display = 'none';
                    }
                } catch (e) {
...
?>
    ...
    <div style="max-width:600px;margin:40px auto;background:#fff;padding:30px;border:1px solid #d0d7de;border-radius:6px;">
        ...
        <div style="margin:20px 0;">
            <strong>Reference:</strong> <?php echo htmlspecialchars($apptref_hex ?? ''); ?><br>
		...
		<div style="margin-top:30px;display:flex;justify-content:center;gap:12px;">
			<a href="frm-appt.html">
				<button class="inputbtn darkblue" type="button">View My Appointments</button>
			</a>
		</div>
    </div>
<?php
